...
Implements MessageReceiver
	...
	#tag Event
		Sub DropObject(obj As DragItem, action As Integer)
		  ...
		  if moveWithin then
		    //erase selected
		    Remove_private(DragTextSelection.offset, DragTextSelection.length, False)
		  end if
		...
		Protected Sub AutocompleteOptionSelected(option as string)
		  ...
		  if option <> "" then
		    if SelLength > 0 then
		      ...
		      if AutocompleteAppliesStandardCase then
		        ...
		        Replace_private(CaretPos - CurrentAutocompleteOptions.CurrentPathComponent.len, CurrentAutocompleteOptions.CurrentPathComponent.len, option)
		      else
		  ...
		  if AutoIndentNewLines then
		    ...
		    if thisLine <> nil and thisLine.isBlockEnd then
		      if IndentLine_private (CaretLine, false) then
	...
	#tag Method, Flags = &h1
		Protected Function CaretColumn() As Integer
	...
	#tag Method, Flags = &h1
		Protected Sub DeleteChars(forwardDelete as boolean)
		  ...
		  if me.SelLength > 0 then
		    replace_private(selStart, me.SelLength, "")
		    Return
		  end if
		  ...
		  if forwardDelete then
		    if CaretPos < TextStorage.Length then
		      if Keyboard.OptionKey then
		        remove_private(selStart, nextNonAlpha(CaretPos) - CaretPos, false)
		      else
		        remove_private(selStart, 1, false)
		      end if
		    end if
		    Return
		  end if
		  ...
		  if Keyboard.OptionKey then
		    ...
		    remove_private(selStart - offset, offset)
		  else
		    remove_private(selStart - 1, 1)
		  end if
		  ...
		  for i as Integer = firstLine to lastLine
		    ...
		    if invalidLines.HasKey(i) or _
		      ...
		      for each tmpSelection in ranges
		        If tmpSelection.IsLineIndexInRange(i) then //if in selection, Highlight line
		          ...
		          if i > tmpSelection.StartLine and i < tmpSelection.EndLine then //fully selected line
		            g.FillRect LineNumOffset, sy - g.TextHeight, g.Width - line.indent, TextHeight
		      ...
		      if SelLength = 0 and i = CaretLine and trailingSuggestion <> "" then
		        g.ForeColor = &cAAAAAA
		        g.DrawString trailingSuggestion, line.indent + AutocompleteSuggestionInsertionX, sy - (g.TextHeight - g.TextAscent)
		      end if
	...
	#tag Method, Flags = &h1
		Protected Function HandleKeyDown(key as string) As boolean
		  ...
		  Else
		    ...
		    if me.SelLength > 0 then
		      replace_private(selStart , me.SelLength, key)
		    else
	...
	#tag Method, Flags = &h0
		Function IndentLine_private(lineIndex as Integer, ltrimLine as Boolean) As Boolean
		  dim line as TextLine = lines.getLine (lineIndex)
		  if line = nil then
		  ...
		  dim previousLine as TextLine, addIndentation as String
		  ...
		  if ltrimLine and currLeadingSpaces > 0 then
		    ...
		    if mIndentVisually then
		      ...
		      Replace_private (line.offset, origLeadingSpaces, "", CurrentEventID, true, true)
		  ...
		  if SyntaxDefinition.isBlockEnd(currTextTrimmed) then
		    ...
		    if mIndentVisually then
		      // Indent by drawing the lines at a further offset, based on the TextLine.indent property
		      ...
		      // Indent by spaces/tabs in the line, which we'll handle below
		    end
		    
		  else
		    // check if the previous line is a block start -> then we increase the indentation in the current line
		    
		    dim indentationIncrease as Integer, prevTxt as String
		    
		    previousLine = lines.getLine(lineIndex - 1)
		    if previousLine <> nil then
		      prevTxt = TextStorage.getText(previousLine.offset, previousLine.length)
		      indentationIncrease = SyntaxDefinition.isBlockStart (prevTxt)
		    end
		    ...
		    if mIndentVisually then
		      ...
		      dim newIndent as Integer
		      if previousLine <> nil then
		        newIndent = previousLine.indent + indentationIncrease * IndentPixels
		      else
		        newIndent = 0
		      end
		      if newIndent <> line.indent then
		      ...
		      addIndentation = indentStr (indentationIncrease)
		    end
		  end
		  
		  // Indent by inserting spaces into the line (which are now in 'addIndentation')
		  
		  line.indent = 0 // reset this value to avoid effects we're not prepared for
		  
		  dim currIndent as String = currTextUntrimmed.Left(currLeadingSpaces)
		  
		  dim prevIdentation as String
		  if previousLine <> nil then
		    // get leading non-whitespace chars from previous line
		    dim firstCharPos as Integer = nextNonWhitespace(previousLine.offset - 1, _
		    previousLine.offset + previousLine.length - previousLine.delimiterLength)
		    prevIdentation = TextStorage.getText(previousLine.offset, firstCharPos - previousLine.offset)
		  end if
		  
		  dim newIndentation as String = prevIdentation + addIndentation
		  ...
		  if theText <> currIndent then
		    ...
		    Replace_private (line.offset, origLeadingSpaces, theText, CurrentEventID, true, true)
		    modified = true
		  end
	...
	#tag Method, Flags = &h1
		Protected Sub receiveMessage(theMessage as Message)
		  ...
		  Elseif theMessage.Sender = highlighter then
		    ...
		    case LineHighlighter.StartReindenting
		      ReindentLinesFrom theMessage.Info(2).IntegerValue, theMessage.Info(3).IntegerValue
	...
	#tag Method, Flags = &h1
		Protected Sub ReindentLinesFrom(fromLine as Integer, atLeastUntilLine as Integer)
		  // indents lines until a line doesn't shift, unless KeepEntireTextIndented=false
		  ...
		  if UndoMgr.isUndoing then break
		  
		  dim needsRedraw as Boolean
		  
		  self.IgnoreRepaint = true
		  for i as Integer = fromLine to lines.Count - 1
		    if not IndentLine_private (i, not IndentVisually) then
		      if i > atLeastUntilLine then
		        exit
		      end if
		    end
		    InvalidateLine (i)
		    needsRedraw = true
		    if not KeepEntireTextIndented then
		      exit
		    end if
		  next
	...
	#tag Method, Flags = &h0
		Sub ReindentText(fromLine as Integer, toLine as integer)
		  ...
		  for i as Integer = fromLine to toLine
		    if IndentLine_private (i, true) then
		      InvalidateLine (i)
		      needsRedraw = true
		    end if
		  next
	...
	#tag Method, Flags = &h0
		Sub Remove_private(offset as integer, length as integer, updateCaret as boolean = true)
	...
	#tag Method, Flags = &h0
		Sub Replace_private(offset as integer, length as integer, text as string, eventID as integer = - 1, keepSelection as Boolean = false, beSilent as Boolean = false)
		  // This method is used internally by the control, and externally by the undo mechanism, you shouldn't use it directly, use instead selstart and seltext.
		  ...
		  UndoMgr.Push(new UndoableReplace(self, offset, removedText, text, CaretPos, eventID))
	...
	#tag Method, Flags = &h1
		Protected Function XposForOffset(line as TextLine, charPos as integer) As single
		  ...
		  Return line.TextWidth(TextStorage, tmp.Graphics, DisplayInvisibleCharacters, CharPos - line.offset) + line.indent
		End Function
	#tag EndMethod
		...
		#tag Setter
			Set
			  if mIndentVisually <> value then
			    ...
			    self.ReindentText
			    self.ResetUndo // this is necessary as Undo won't work right otherwise (not sure why)
			  end if
			End Set
		#tag EndSetter
	...
	#tag ComputedProperty, Flags = &h0
		...
		#tag Setter
			Set
			  CurrentEventID = 0 // -> single undo
			  Replace_private(selStart, selLength, value)
			End Set
		#tag EndSetter
		SelText As string
	#tag EndComputedProperty
		...
		#tag Setter
			Set
			  ...
			  if not disableReset then
			    ...
			    ' Auto Reset the property
			    UndoMgr.Push(new UndoableReplace(self, 0, self.text, value, CaretPos,  CurrentEventID))
			    disableReset = False
			  end if
